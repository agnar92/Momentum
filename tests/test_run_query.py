...
class TestComputeMansfieldRsChart:
    ...
    def test_no_stock_history_returns_none(self):
        con = make_gem_con()
        insert_weekly_series(con, "index_prices", "Index_Name", "NASDAQ100", "2025-01-06", 60, 200.0, 0.3)
        assert compute_mansfield_rs_chart(con, "NOPE", "NASDAQ100", "2026-03-30", "2025-10-06") is None

    def test_no_index_history_returns_none(self):
        con = make_gem_con()
        insert_weekly_series(con, "prices", "Ticker", "AAA", "2025-01-06", 60, 100.0, 1.0)
        assert compute_mansfield_rs_chart(con, "AAA", "NASDAQ100", "2026-03-30", "2025-10-06") is None
...
class TestComputeTtmSqueezeChart:
    def test_long_consolidation_then_breakout_is_detected(self):
        ...
        # momentu jest juz >= 2*TTM_SQUEEZE_KC_WEEKS+2 (22) tyg. historii, wiec BB/KC
    ...
    def test_insufficient_history_leaves_squeeze_fields_none(self):
        con = make_gem_con()
        fixture_start = pd.Timestamp("2025-01-06")
        # Tylko 5 tyg. — za malo na 10-tyg. BB/KC (TTM_SQUEEZE_KC_WEEKS).
        closes = [100.0 + i * 0.1 for i in range(5)]
        insert_weekly_ohlc_close_list(con, "prices", "Ticker", "AAA", fixture_start.strftime("%Y-%m-%d"), closes)

        ref_date = fixture_start + pd.Timedelta(weeks=4)
        out = compute_ttm_squeeze_chart(con, "AAA", "NASDAQ100", ref_date.strftime("%Y-%m-%d"),
