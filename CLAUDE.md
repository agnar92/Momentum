...
- **`js/qol.js`** — a third small shared `<script>` file, loaded on ALL THREE pages (before `js/app.js` on
  `index.html`, before `js/rebalance.js` on `rebalance.html`, before `js/chart.js` on `chart.html`),
  holding three independent, user-facing quality-of-life pieces the app didn't have before:
  ...
  "undefined"` → `Object.assign(globalThis, require("./qol.js"))` guard so the cross-file globals resolve
