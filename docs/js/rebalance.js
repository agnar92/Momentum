...
function renderPickerTable() {
    ...
    rows.forEach(c => {
        ...
        // stopPropagation nizej) otwiera wlasny wykres spolki — dokladnie jak
        // klikniecie wiersza w glownej tabeli dashboardu (selectTicker w app.js).
        tr.addEventListener("click", () => showPickerChart(c.ticker, universe));
        tbody.appendChild(tr);
    });
...
// ============================================================
// WYKRES SPÓŁKI W KROKU 2 — kliknięcie wiersza tabeli (patrz renderPickerTable
// powyżej; przycisk "+ Dodaj"/"✓ W portfelu" zostaje NIETKNIĘTY, działa
// niezależnie — patrz e.stopPropagation() przy pick-toggle-btn) pokazuje ten
// sam własny wykres stage-analysis co dashboard (patrz renderRelativeStrengthChart
// w app.js): "wykres 10:30" (cena + SMA10/SMA30 + VWAP + poziom indeksu, w %
// zmiany od początku okna, z bazami Darvasa, interaktywny zoom/pan), panel
// wolumenu kupujących/sprzedających, oscylator Mansfield RS i wskaźnik TTM
// Squeeze — te same pola (weekly_chart/mansfield_chart/ttm_squeeze_chart) już
// dostępne w KAŻDYM rekordzie all_constituents, więc nie trzeba nic
// doczytywać. Celowo zduplikowane z app.js (strona nie ma wspólnego modułu —
// patrz komentarz przy STAGE_LABELS wyżej), i celowo BEZ trybu pełnoekranowego
// i przełącznika zakresu 3 mies./cały zakres z dashboardu — ten ekran nie ma
// tej samej presji "jeden ekran naraz" na telefonie, więc uproszczone do
// zawsze pełnego (~14-miesięcznego) zakresu.
// ============================================================
...
const STAGE_BREAKOUT_VOLUME_RATIO = 1.5; // musi byc zgodne z run_query.py::STAGE_BREAKOUT_VOLUME_RATIO
const VOLUME_SMA_WEEKS = 10; // ~50 dni sesyjnych

function pickerTvSymbolFor(ticker, universe) {
    return PLN_UNIVERSES.has(universe) ? `GPW:${ticker}` : ticker;
}
function pickerTvUrlFor(ticker, universe) {
    return `https://www.tradingview.com/chart/?symbol=${encodeURIComponent(pickerTvSymbolFor(ticker, universe))}`;
}

// Ten sam odczyt co findRsEntry w app.js — ticker jest szukany we WŁASNYM
// uniwersum wpisu (a nie tylko przeglądanym w Kroku 1), bo picks mogą
// pochodzić z dowolnego z 5 uniwersów.
function findPickerRsEntry(ticker, universe) {
    const data = universeData[universe] || {};
    const list = data.all_constituents || data.constituents || [];
    const entry = list.find(c => c.ticker === ticker);
    return (entry && entry.weekly_chart) ? { ...entry, universe } : null;
}
...
function renderPickerStageBadge(stage) {
    const badge = document.getElementById("pickerStageBadge");
    if (!badge) return;
...
let pickerRsChartInstance = null;
let pickerRsVolumeChartInstance = null;
let pickerRsMansfieldChartInstance = null;
let pickerRsSqueezeChartInstance = null;
let pickerChartTicker = null;
let pickerChartUniverse = null;

function syncPickerVolumeXRange(sourceChart) {
    if (!pickerRsVolumeChartInstance) return;
    const xScale = sourceChart.scales.x;
    pickerRsVolumeChartInstance.options.scales.x.min = xScale.min;
    pickerRsVolumeChartInstance.options.scales.x.max = xScale.max;
    pickerRsVolumeChartInstance.update("none");
}

function resetPickerChartZoom() {
    if (pickerRsChartInstance) pickerRsChartInstance.resetZoom();
    if (pickerRsVolumeChartInstance) {
        pickerRsVolumeChartInstance.options.scales.x.min = undefined;
        pickerRsVolumeChartInstance.options.scales.x.max = undefined;
        pickerRsVolumeChartInstance.update();
    }
}

function renderPickerRelativeStrengthChart(symbol, rsEntry) {
    ...
    const canvas = document.getElementById("pickerRsChartCanvas");
    const volumeCanvas = document.getElementById("pickerRsVolumeCanvas");
    const mansfieldCanvas = document.getElementById("pickerRsMansfieldCanvas");
    const squeezeCanvas = document.getElementById("pickerRsSqueezeCanvas");
    if (!canvas || !chartData || typeof Chart === "undefined") return;

    if (pickerRsChartInstance) { pickerRsChartInstance.destroy(); pickerRsChartInstance = null; }
    if (pickerRsVolumeChartInstance) { pickerRsVolumeChartInstance.destroy(); pickerRsVolumeChartInstance = null; }
    if (pickerRsMansfieldChartInstance) { pickerRsMansfieldChartInstance.destroy(); pickerRsMansfieldChartInstance = null; }
    if (pickerRsSqueezeChartInstance) { pickerRsSqueezeChartInstance.destroy(); pickerRsSqueezeChartInstance = null; }

    renderPickerStageBadge(chartData.current_stage);
    ...
    pickerRsChartInstance = new Chart(canvas, {
        type: "line",
        data: {
            labels: chartData.dates,
            datasets: [
                { label: `${symbol} (zmiana %)`, data: chartData.close_pct, borderColor: "#2ecc71", backgroundColor: "transparent", pointRadius: 0, borderWidth: 2, order: 1 },
        ...
        options: {
            ...
            plugins: {
                ...
                zoom: {
                    ...
                    pan: { enabled: true, mode: "x", onPanComplete: ({ chart }) => syncPickerVolumeXRange(chart) },
                    zoom: {
                        wheel: { enabled: true }, pinch: { enabled: true }, mode: "x",
                        onZoomComplete: ({ chart }) => syncPickerVolumeXRange(chart),
                    },
                },
            },
            scales: {
    ...
    if (volumeCanvas) {
        ...
        pickerRsVolumeChartInstance = new Chart(volumeCanvas, {
            type: "bar",
            data: {
                labels: chartData.dates,
                datasets: [
                    ...
                    { type: "line", label: `Śr. wolumenu (${VOLUME_SMA_WEEKS} tyg. ≈ 50 dni)`, data: volumeSma, borderColor: "#e0a72e", backgroundColor: "transparent", pointRadius: 0, borderWidth: 1.5, borderDash: [3, 2], order: 0 },
                ],
            },
            options: {
                responsive: true,
    ...
    const mansfieldCaption = document.getElementById("pickerRsMansfieldCaption");
    if (mansfieldCanvas && mansfieldData) {
        ...
        pickerRsMansfieldChartInstance = new Chart(mansfieldCanvas, {
            type: "line",
            data: {
                labels: chartData.dates,
                datasets: [
    ...
    const squeezeCaption = document.getElementById("pickerRsSqueezeCaption");
    if (squeezeCanvas && squeezeData) {
        ...
        pickerRsSqueezeChartInstance = new Chart(squeezeCanvas, {
            type: "bar",
            data: {
                labels: chartData.dates,
                datasets: [
                    { type: "bar", label: "Momentum (histogram)", data: aligned.histogram, backgroundColor: histColors, borderWidth: 0, order: 2 },
                    { type: "line", label: "Squeeze", data: zeroLineSqueeze, showLine: false, pointRadius: 4, pointHoverRadius: 5, pointBackgroundColor: dotColors, pointBorderWidth: 0, order: 1 },
                ],
            },
            options: {
                responsive: true,
...
function updatePickerChartTickerLabel() {
    const label = document.getElementById("pickerChartTickerLabel");
    if (!label) return;
    if (!pickerChartTicker) { label.textContent = ""; return; }
    const parts = [pickerChartTicker];
    if (pickerChartUniverse && UNIVERSE_LABELS[pickerChartUniverse]) parts.push(UNIVERSE_LABELS[pickerChartUniverse]);
    label.textContent = parts.join(" · ");
}

function renderPickerChartArea() {
    const rsEntry = findPickerRsEntry(pickerChartTicker, pickerChartUniverse);
    const hasChart = !!(rsEntry && rsEntry.weekly_chart);

    const noChartMsg = document.getElementById("pickerNoChartMessage");
    const rsChartPanel = document.getElementById("pickerRsChartPanel");
    const rsVolumePanel = document.getElementById("pickerRsVolumePanel");
    const rsMansfieldPanel = document.getElementById("pickerRsMansfieldPanel");
    const rsSqueezePanel = document.getElementById("pickerRsSqueezePanel");
    const stageLegend = document.getElementById("pickerStageLegend");
    if (noChartMsg) noChartMsg.hidden = hasChart;
    if (rsChartPanel) rsChartPanel.hidden = !hasChart;
    if (rsVolumePanel) rsVolumePanel.hidden = !hasChart;
    if (rsMansfieldPanel) rsMansfieldPanel.hidden = !hasChart;
    if (rsSqueezePanel) rsSqueezePanel.hidden = !hasChart;
    if (stageLegend) stageLegend.hidden = !hasChart;

    if (hasChart) {
        renderPickerRelativeStrengthChart(pickerChartTicker, rsEntry);
    } else {
        renderPickerStageBadge(null);
        if (pickerRsChartInstance) { pickerRsChartInstance.destroy(); pickerRsChartInstance = null; }
        if (pickerRsVolumeChartInstance) { pickerRsVolumeChartInstance.destroy(); pickerRsVolumeChartInstance = null; }
        if (pickerRsMansfieldChartInstance) { pickerRsMansfieldChartInstance.destroy(); pickerRsMansfieldChartInstance = null; }
        if (pickerRsSqueezeChartInstance) { pickerRsSqueezeChartInstance.destroy(); pickerRsSqueezeChartInstance = null; }
    }
    updatePickerChartTickerLabel();
}

function showPickerChart(ticker, universe) {
    pickerChartTicker = ticker;
    pickerChartUniverse = universe;
    const tableView = document.getElementById("pickerTableView");
    const chartView = document.getElementById("pickerChartView");
    if (tableView) tableView.hidden = true;
    if (chartView) chartView.hidden = false;
    renderPickerChartArea();
}

function hidePickerChart() {
    const tableView = document.getElementById("pickerTableView");
    const chartView = document.getElementById("pickerChartView");
...
function initPickerChartControls() {
    const backBtn = document.getElementById("pickerChartBackBtn");
    if (backBtn) backBtn.addEventListener("click", hidePickerChart);
    const tvBtn = document.getElementById("pickerOpenTvBtn");
    if (tvBtn) {
        tvBtn.addEventListener("click", () => {
            if (!pickerChartTicker) return;
            window.open(pickerTvUrlFor(pickerChartTicker, pickerChartUniverse), "_blank", "noopener");
        });
    }
    const resetBtn = document.getElementById("pickerResetZoomBtn");
    if (resetBtn) resetBtn.addEventListener("click", resetPickerChartZoom);
}
